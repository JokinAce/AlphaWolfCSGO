...
public class VAMemory {
    ...
    public bool CheckProcess() {
        ...
        processHandle = VAMemory.OpenProcess(2035711U, false, mainProcess[0].Id);
    ...
    public byte[] ReadByteArray(IntPtr pOffset, uint pSize) {
        ...
        byte[] result;
        try {
            VAMemory.VirtualProtectEx(processHandle, pOffset, (UIntPtr)pSize, 4U, out uint flNewProtect);
            byte[] array = new byte[pSize];
            VAMemory.ReadProcessMemory(processHandle, pOffset, array, pSize, 0U);
            VAMemory.VirtualProtectEx(processHandle, pOffset, (UIntPtr)pSize, flNewProtect, out flNewProtect);
            result = array;
        } catch (Exception ex) {
    ...
    public bool WriteByteArray(IntPtr pOffset, byte[] pBytes) {
        ...
        bool result;
        try {
            VAMemory.VirtualProtectEx(processHandle, pOffset, (UIntPtr)((ulong)pBytes.Length), 4U, out uint flNewProtect);
            bool flag = VAMemory.WriteProcessMemory(processHandle, pOffset, pBytes, (uint)pBytes.Length, 0U);
            VAMemory.VirtualProtectEx(processHandle, pOffset, (UIntPtr)((ulong)pBytes.Length), flNewProtect, out flNewProtect);
            result = flag;
        } catch (Exception ex) {
            if (VAMemory.debugMode) {
